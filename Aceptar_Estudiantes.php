<?php
    ...
    // Consulta para obtener el correo electrónico
    $sql = "SELECT dp.Correo_Electronico, dp.Segundo_Nombre, dp.Primer_Apellido, dp.Segundo_Apellido, ip.Fecha_De_inicio, ip.Fecha_De_Finalización FROM Dtos_Personales dp JOIN Informacion_Practicas ip ON dp.ID = ip.ID_Practicante WHERE dp.ID = ?";
    ...
    if ($result->num_rows > 0) {
        ...
        $correo_electronico = $row['Correo_Electronico']; // Asignamos el correo electrónico
        $segundo_nombre = $row['Segundo_Nombre']; 
        $primer_apellido = $row['Primer_Apellido']; 
        $segundo_apellido = $row['Segundo_Apellido']; 
        $fecha_inicio = $row['Fecha_De_inicio']; 
        $fecha_final = $row['Fecha_De_Finalización']; 
    } else {
        die("No se encontró el correo electrónico para el ID del aspirante proporcionado.");
    }

    // Procesar la acción POST
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        if ($_POST['action'] === 'aceptar') {
            header('Location: /aceptar-practicante?id_aspirante=' . $id_aspirante . '&primer_nombre=' . $primer_nombre . '&segundo_nombre=' . $segundo_nombre . '&primer_apellido=' . $primer_apellido . '&segundo_apellido=' . $segundo_apellido . '&fecha_inicio=' . $fecha_inicio . '&fecha_final=' . $fecha_final . '&correo_electronico=' . urlencode($correo_electronico));
            exit();
    ...
    <div class="container">
        ...
        <form action="" method="POST">
                    ...
                    <div class="tab-pane fade show active" id="info-aspirante" role="tabpanel" aria-labelledby="info-aspirante-tab">
                            ...
                            if ($result2->num_rows > 0) {
                                while($row = $result2->fetch_assoc()) {
                                    ...
                                    echo '<input type="text" class="form-control" name="Fecha_De_inicio" value="' . $fecha_inicio . '" placeholder="Fecha de inicio" readonly>';
                                    echo '</div>';
                                    echo '<div class="mb-2">';
                                    echo '<label for="Fecha_De_Finalización">Fecha de Finalización:</label>';
                                    echo '<input type="text" class="form-control" name="Fecha_De_Finalización" value="' . $fecha_fin . '" placeholder="Fecha de Finalización" readonly>';
                                    echo '</div>';
                                    echo '<div class="mb-2">';
                    ...
                    <div class="tab-pane fade" id="info-personal" role="tabpanel" aria-labelledby="info-personal-tab">
                        ...
                        if ($result->num_rows > 0) {
                            while($row = $result->fetch_assoc()) {
                                ...
                                echo '<input type="date" class="form-control" name="fecha_de_nac" value="' . $fecha_nacimiento . '" placeholder="Fecha_de_Nac" readonly>';
                                echo '</div>';
                                echo '<div class="mb-2">';
            ...
            <div class="d-flex justify-content-between mt-4">
                <button type="button" class="btn-denegar" data-bs-toggle="modal" data-bs-target="#denegarModal">Denegar</button>
          ...
          <div class="modal-dialog">
            <div class="modal-content">
              <div class="modal-header">
                <h5 class="modal-title" id="denegarModalLabel">Motivo de la Denegación</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
              </div>
              <div class="modal-body">
                <form id="denegarForm" method="POST" action="procesar_denegacion.php">
                  <div class="mb-3">
                    <label for="motivo" class="form-label">Escribe el motivo de la denegación:</label>
                    <textarea class="form-control" id="motivo" name="motivo" rows="3" required></textarea>
                  </div>
                  <?php
                            // Consulta para Información Personal
                            $sql = "SELECT Correo_Electronico, Primer_Nombre, Segundo_Nombre, Primer_Apellido, Segundo_Apellido FROM Dtos_Personales WHERE ID = ?";
                            $stmt = $conn->prepare($sql);
                            $stmt->bind_param("s", $id_aspirante);
                            $stmt->execute();
                            $result = $stmt->get_result();
                        ...
                        if ($result->num_rows > 0) {
                            while($row = $result->fetch_assoc()) {
                                echo '<input type="hidden" name="primer_nombre" value="' . $row["Primer_Nombre"] . '">';
                                echo '<input type="hidden" name="segundo_nombre" value="' . $row["Segundo_Nombre"] . '">';
                                echo '<input type="hidden" name="primer_apellido" value="' . $row["Primer_Apellido"] . '">';
                                echo '<input type="hidden" name="segundo_apellido" value="' . $row["Segundo_Apellido"] . '">';
                                echo '<input type="hidden" name="correo_electronico" value="' . $row["Correo_Electronico"] . '">';
                                echo '<input type="hidden" name="id_aspirante" value="' . $id_aspirante . '">';
                            }
                        }
                    ?>
