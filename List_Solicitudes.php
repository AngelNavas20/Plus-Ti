...
<head>
    ...
    <style>
        ...
        .btn-revisar .material-symbols-outlined {
            font-size: 30px;
        }
        .btn-primary {
...
<div class="containersearch">
    <p style="font-size: 20px; color: #071359; margin-bottom: 13px" class="text-center"><strong>Listado de solicitudes de prácticas pendientes</strong></p>
    <form id="searchForm" action="/solicitudes-pendientes" method="GET" class="mb-3 text-right">
        <div class="SearchAndShow block">
            <div class="mx-auto">
                ...
                <a href="/solicitudes-pendientes" class="btn-ver">Ver todos</a>
            </div>
        </div>
    </form>
</div>
    ...
    <div class="modal-dialog">
        <div class="modal-content rounded-3xl">
            <div class="modal-body" style="margin: 20px; max-width: 100%;">
                <form id="filterForm" action="/solicitudes-pendientes" method="GET">
                    <div class="mb-3">
...
<div class="container3 text-center">
    <table class="table">
        <thead>
            <tr>
                <th class="col-22">Nombre completo del solicitante</th>
        ...
        <tbody>
           ...
           // Definir la consulta SQL base
            $sql = "SELECT ID, Primer_Nombre, Segundo_Nombre, Primer_Apellido, Segundo_Apellido, Institucion_Colegio, Grado, Carrera, Habilidades
            FROM Dtos_Personales dp JOIN Informacion_Practicas ip ON dp.ID = ip.ID_Practicante WHERE ip.Estado='Pendiente'";
            ...
            if (isset($_GET['searchQuery']) && !empty($_GET['searchQuery'])) {
                ...
                // Crear la cláusula WHERE adicional dinámicamente para buscar en varios campos
                $whereClauses = [];
                foreach ($searchTerms as $term) {
                    $whereClauses[] = "(dp.Primer_Nombre LIKE '%$term%' OR dp.Segundo_Nombre LIKE '%$term%' OR dp.Primer_Apellido LIKE '%$term%' OR dp.Segundo_Apellido LIKE '%$term%' OR ip.Institucion_Colegio LIKE '%$term%' OR ip.Grado LIKE '%$term%' OR ip.Carrera LIKE '%$term%' OR ip.Habilidades LIKE '%$term%')";
                }
            
                // Agregar las cláusulas de búsqueda a la consulta SQL
                $sql .= " AND (" . implode(' AND ', $whereClauses) . ")";
            
                // Registrar la operación en Log_Json para el SELECT con filtro
            ...
            if ($result->num_rows > 0) {
                while($row = $result->fetch_assoc()) {
                    ...
                    echo "<td class='text-center'><button class='btn-revisar' onclick=\"window.location.href='/informacion-aspirante?id_aspirante=" . $row['ID'] . "&primer_nombre=" . $row['Primer_Nombre'] . "&from=solicitudes-pendientes';\" title='Revisar solicitud'><span class='material-symbols-outlined'>quick_reference_all</span></button></td>";
                    echo "</tr>";
            ...
            $countSql = "SELECT COUNT(*) as total FROM Dtos_Personales dp JOIN Informacion_Practicas ie ON dp.ID = ie.ID_Practicante WHERE Estado='Pendiente'";
            if (isset($_GET['searchQuery']) && !empty($_GET['searchQuery'])) {
